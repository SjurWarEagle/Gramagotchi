...
func remove_old_gremlins():
	var gremlin = get_node("/root/world/Gremlin")
	world.remove_child(gremlin)
	gremlin.queue_free()
...
func http_request_dna_completed(_result, _response_code, _headers, body, dna_http_request:HTTPRequest, gremlin_name: String):
	...
	http_request.request_completed.connect(self.http_request_image_completed.bind(http_request, gremlin_name))
	# Perform the HTTP request. The URL below returns a PNG image as of writing.
	var url = "https://gremlin.tkunkel.de/api/generate-gremlin?colourSkin=0x%x&colourFurFront=0x%x&colourFurBack=0x%x&size=200" % [gremlinDna.colourSkin,
	gremlinDna.colourFurFront,gremlinDna.colourFurBack];
	print("getting "+url)
